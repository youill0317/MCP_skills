...
function printSummary(io, summary, check, skillsRoot) {
    const modeLabel = check ? "check" : "apply";
    io.stdout(`Skills root: ${skillsRoot}`);
    io.stdout(`Mode: ${modeLabel}`);
    io.stdout(`Summary: updated=${summary.updated} unchanged=${summary.unchanged} invalid=${summary.invalid} missing=${summary.missing}`);
    for (const record of summary.records) {
        const detail = record.message ? ` (${record.message})` : "";
        io.stdout(`- ${record.skillId}: ${record.status}${detail}`);
    }
}
export async function runFrontmatterMigrationCli(argv, io) {
    try {
